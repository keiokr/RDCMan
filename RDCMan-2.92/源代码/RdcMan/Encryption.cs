...
namespace RdcMan
{
	public static class Encryption
	{
		...
		public static bool LegacyPasswordsMigrated { get; private set; }
		...
		public static string EncryptionMethodToString(EncryptionMethod method)
		{
			return method switch
			{
				...
				EncryptionMethod.LogonCredentials => "便携密钥（当前目录）",
		...
		public static string EncryptString(string plaintext, EncryptionSettings settings)
		{
			...
			case EncryptionMethod.LogonCredentials:
				return PortableCrypto.Encrypt(plaintext);
		...
		public static string DecryptString(string encryptedString, EncryptionSettings settings)
		{
			...
			{
			case EncryptionMethod.LogonCredentials:
				if (encryptedString.StartsWith("RDCManPortable1:", StringComparison.Ordinal))
				{
					return PortableCrypto.Decrypt(encryptedString);
				}
				// 兼容旧版 RDCMan 的本机 DPAPI 密文。保存后会自动转换为便携密文。
				string plaintext = DecryptStringUsingLocalUser(encryptedString);
