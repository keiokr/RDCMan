...
namespace RdcMan
{
	internal class Program
	{
		...
		private static void ParseCommandLine()
		{
			...
			if (_openFiles && _filesToOpen.Count == 0)
			{
				string[] rdgFiles = Directory.GetFiles(PortablePaths.BaseDirectory, "*.rdg", SearchOption.TopDirectoryOnly)
					.OrderBy(path => Path.GetFileName(path).Equals("mstsc.rdg", StringComparison.OrdinalIgnoreCase) ? 0 : 1)
					.ThenBy(path => path, StringComparer.OrdinalIgnoreCase)
					.ToArray();
				_filesToOpen.AddRange(rdgFiles);
			}
		}
